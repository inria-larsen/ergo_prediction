...
class mvnx_tree():
	...
	def __init__(self, path):
		...
		try:
			...
			mvnx_tree = etree.parse(path,  parser=parser)
	...
	def get_npose(self):
		...
		position_npose = []
		for frame in frame_list:
			if 	frame.get('type')=='npose':
				orientation_npose = next(frame.iterfind(self.prefix+'orientation')).text.split()
				position_npose = next(frame.iterfind(self.prefix+'position')).text.split()
		orientation_npose = np.asarray(orientation_npose)
		position_npose = np.asarray(position_npose)
		return orientation_npose.astype(np.float), position_npose.astype(np.float)
